...
-- The script gives out the supply of hotels for a given country in 2020 and 2021, across segments.
			...
			(select
					...
					FROM (
						...
						SELECT * FROM (
						  ...
						  FROM Booking2021 where (STARS_ is not NULL and STARS_<>'nan') and TYPE_ LIKE '%Hôtel%' and PAYS=@input_2021 group by CAST(STARS_ AS float)
						) RESULTS2021 FULL JOIN
							(SELECT
								COUNT(URL_) NB_2020,
								CAST(NBETOILE AS float) STARS_N2020
							FROM Booking2020 where (NBETOILE is not NULL and NBETOILE<>0) and TYPE_ETABLISSEMENT LIKE '%Hôtel%' and URL_ LIKE @input_2020 group by CAST(NBETOILE AS float)
						) RESULTS2020
